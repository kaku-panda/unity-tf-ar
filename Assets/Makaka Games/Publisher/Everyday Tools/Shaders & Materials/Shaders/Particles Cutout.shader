...
Shader "Custom/Makaka Games/Particles/Cutout Transparent"
{
	...
	SubShader
	{
		...
		inline half4 LightingStandardCustom(
			SurfaceOutputStandardCustom s, half3 viewDir, UnityGI gi)
		{
			float3 lightAtten =
				lerp(_LightColor0.rgb, gi.light.color, _TransShadow);
			
			half3 lightDir = gi.light.dir + s.Normal * _TransNormalDistortion;
			
			half transVdotL =
				pow(saturate(dot(viewDir, -lightDir)), _TransScattering);
			
			half3 translucency = lightAtten
				* (transVdotL * _TransDirect
					+ gi.indirect.diffuse * _TransAmbient)
				* s.Translucency;
